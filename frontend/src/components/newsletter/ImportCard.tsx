...
function jobFromState(s: LiveState): ImportJob | null {
  if (s.phase === 'tracking' || s.phase === 'completed') return s.job
  if (s.phase === 'failed' && s.job) return s.job
  return null
}
    ...
    const id = setInterval(async () => {
      try {
        ...
        if (next.status === 'completed') {
          setLive({ phase: 'completed', job: next })
          void refreshHistory()
          clearInterval(id)
        } else if (next.status === 'failed') {
          setLive({ phase: 'failed', error: 'Import failed', job: next })
          void refreshHistory()
          clearInterval(id)
        } else {
  ...
  return (
    <div className="bg-white border rounded-2xl shadow-soft p-5 space-y-5">
      ...
      <div
        ...
        className="rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50/50 hover:bg-gray-50 transition p-6"
      >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <label className="block">
          ...
          <input
            ...
            className="mt-1 w-full rounded-xl border px-3 py-2"
            placeholder="e.g. CRM export Sep 2025"
          />
        </label>
      ...
      {job && (live.phase === 'tracking' || live.phase === 'completed' || live.phase === 'failed') && (
        <div className="rounded-2xl border p-4">
          <div className="flex items-center justify-between mb-3">
            <div className="font-medium">Import Status</div>
            <div className="text-sm text-gray-500">Created: {fmtDate(job.created_at)}</div>
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 md:grid-cols-5 gap-3 text-sm">
            {(['rows_total','rows_created','rows_updated','rows_skipped','rows_errors'] as const).map((k) => (
              <div key={k} className="bg-gray-50 rounded-xl p-2">
                ...
                <div className="font-semibold">{job[k]}</div>
              </div>
            ))}
          </div>
          <div className="mt-3 flex items-center justify-between">
            ...
            {job.error_report ? (
              <a className="px-3 py-2 rounded-xl border hover:bg-gray-50 text-sm" href={job.error_report} target="_blank" rel="noreferrer">
                Download Errors CSV
              </a>
        ...
        {history.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-separate border-spacing-y-2">
              ...
              <tbody>
                {history.map((h) => {
                  const pct = progressPercent(h.rows_total, h.rows_created, h.rows_updated, h.rows_skipped, h.rows_errors)
                  return (
                    <tr key={h.id} className="bg-white hover:bg-gray-50">
                      <td className="px-3 py-3 text-sm">#{h.id}</td>
                      <td className="px-3 py-3 text-sm"><Badge tone={h.status === 'completed' ? 'green' : h.status === 'failed' ? 'red' : 'blue'}>{h.status}</Badge></td>
                      <td className="px-3 py-3 text-sm">{fmtDate(h.created_at)}</td>
                      ...
                      <td className="px-3 py-3 text-sm text-right">
                        <div className="space-x-2">
                          <span className="text-green-700">+{h.rows_created}</span>
                          <span className="text-blue-700">~{h.rows_updated}</span>
                          <span className="text-gray-500">•{h.rows_skipped}</span>
                          <span className="text-red-600">!{h.rows_errors}</span>
                        </div>
                      </td>
                      <td className="px-3 py-3 text-sm text-right">
                        {h.error_report ? (
                          <a href={h.error_report} className="text-xs px-2 py-1 rounded-lg border hover:bg-gray-50" target="_blank" rel="noreferrer">Download</a>
                        ) : <span className="text-xs text-gray-400">—</span>}
                      </td>
                    </tr>
                  )
