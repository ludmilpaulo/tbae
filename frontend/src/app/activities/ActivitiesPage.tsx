...
export default function ActivitiesPage() {
  ...
  // ✅ these are arrays thanks to transformResponse
  const { data: categories = [] } = useGetActivityCategoriesQuery();
  const { data: allActivities = [], isLoading } = useGetActivitiesQuery();

  const filteredActivities = useMemo<Activity[]>(() => {
    if (!selected) return allActivities;
    return allActivities.filter((a) => a.category?.id === selected);
  }, [allActivities, selected]);

  const totalPages = Math.ceil(filteredActivities.length / ITEMS_PER_PAGE);

  const paginatedActivities = useMemo<Activity[]>(() => {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-14">
      ...
      <ActivityFilters categories={categories as ActivityCategory[]} selected={selected} onSelect={handleSelect} />
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-7 mt-8">
        {isLoading ? (
          ...
          paginatedActivities.map((activity) => <ActivityCard key={activity.id} activity={activity} />)
        )}
      </div>
        ...
        <div className="flex justify-center mt-12 space-x-2">
          <button
            ...
            className={`px-4 py-2 rounded-full font-bold transition border ${
              page === 1 ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-white text-blue-700 border-blue-200 hover:bg-blue-50"
            }`}
          >
            Prev
          </button>
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <button
              ...
              onClick={() => setPage(p)}
              className={`px-4 py-2 rounded-full font-bold transition border ${
                page === p ? "bg-blue-600 text-white shadow" : "bg-white text-blue-700 border-blue-200 hover:bg-blue-50"
              }`}
            >
              {p}
            </button>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full font-bold transition border ${
              page === totalPages ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-white text-blue-700 border-blue-200 hover:bg-blue-50"
            }`}
          >
            Next
          </button>
