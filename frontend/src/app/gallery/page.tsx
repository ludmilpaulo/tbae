// src/app/gallery/page.tsx (or wherever GalleryPage lives)
"use client";
...
import type { GalleryCategory, GalleryEvent, GalleryPhoto, GalleryVideo } from "@/types/gallery";

function getYouTubeId(url: string): string | null {
  const m = url.match(/(?:youtu\.be\/|youtube\.com(?:\/embed\/|\/watch\?v=|\/v\/|\/shorts\/))([\w\-]+)/);
  return m ? m[1] : null;
}
...
export default function GalleryPage() {
  ...
  // Hooks (now normalized by transformResponse)
  const { data: rawCategories = [] } = useGetGalleryCategoriesQuery();
  const categories: GalleryCategory[] = Array.isArray(rawCategories) ? rawCategories : [];

  const { data: rawYears = [] } = useGetGalleryYearsQuery();
  const years: number[] = Array.isArray(rawYears) ? rawYears : [];
  ...
  useEffect(() => {
    if (page === 1) {
      ...
      setGallery(prev => prev.concat(data.results));
    }
    ...
    <main className="max-w-7xl mx-auto px-2 md:px-4 py-10">
      ...
      <div className="flex flex-wrap gap-2 mb-4 items-center">
        ...
        {categories.map((cat) => (
          ...
          >
            {eventTypeLabels[cat.name] ?? (cat as { display_name?: string }).display_name ?? cat.name}
          </button>
        ))}
      ...
      <div className="columns-1 sm:columns-2 md:columns-3 gap-4 space-y-4">
        {gallery.flatMap((item) => [
          ...(Array.isArray(item.photos) ? item.photos : []).map((photo: GalleryPhoto) => (
            <div
              key={`photo-${photo.id}`}
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/60 to-transparent px-4 py-2 text-xs md:text-sm text-white opacity-0 group-hover:opacity-100 transition">
                ...
                <div className="italic text-xs mt-1">{item.event_type && (eventTypeLabels[item.event_type] ?? item.event_type)}</div>
              </div>
            </div>
          )),
          ...(Array.isArray(item.videos) ? item.videos : []).map((video: GalleryVideo) => {
      ...
      {modal && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center" onClick={() => setModal(null)}>
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            {modal.type === "photo" ? (
              <>
                <Image src={modal.url} alt={modal.caption || ""} width={1200} height={900} className="rounded-2xl shadow-2xl max-h-[80vh] w-auto" priority />
                {modal.caption && <div className="mt-4 text-center text-white text-lg font-semibold drop-shadow">{modal.caption}</div>}
              </>
            ) : (
              <>
