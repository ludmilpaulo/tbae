...
class BookingCreateAPIView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"detail": "Venue not found."}, status=status.HTTP_400_BAD_REQUEST)
        # Validate dates
        try:
        ...
        if nights <= 0:
            return Response({"detail": "Check-out must be after check-in."}, status=status.HTTP_400_BAD_REQUEST)
        # Calculate price
        ...
        # Send email to user
        try:
            html_content = render_to_string("bookings/booking_confirmation_email.html", {
                "booking": booking,
                "venue": venue,
            ...
            msg = EmailMultiAlternatives(subject, "", settings.DEFAULT_FROM_EMAIL, [booking.email])
